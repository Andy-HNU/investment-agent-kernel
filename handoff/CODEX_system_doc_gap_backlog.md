...
- add-cash candidates now honor `new_cash_split_buckets` across multiple underweight targets, with min-threshold clipping tolerant to float-edge exact hits
...
- execution-plan approval / supersede / confirmation 已在 frontdesk 显式落地，但仍未上升为 orchestrator-managed state machine
...
- frontdesk service / CLI now expose explicit plan approval:
