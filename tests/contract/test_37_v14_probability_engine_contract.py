...
def test_market_pressure_snapshot_fields_are_preserved() -> None:
    snapshot = MarketPressureSnapshot(
    ...
    assert snapshot.scenario_kind == "current_market"
    assert snapshot.market_pressure_score == 42.0
    assert snapshot.market_pressure_level == "L1_中性偏紧"
    assert snapshot.current_regime == "risk_off"
    assert snapshot.systemic_jump_dispersion_multiplier == 1.05


def test_probability_engine_output_preserves_direct_pressure_objects() -> None:
    ...
    assert output.current_market_pressure is pressure
    assert output.current_market_pressure.market_pressure_level == "L1_中性偏紧"
    assert output.scenario_comparison == [scenario]
    assert output.scenario_comparison[0].pressure is pressure
    assert output.scenario_comparison[0].recipe_result is primary_result


def test_probability_engine_output_round_trips_pressure_structures() -> None:
    primary_result = RecipeSimulationResult(
        recipe_name="scheme_b_primary",
        role="primary",
        success_probability=0.71,
    ...
    rehydrated = ProbabilityEngineOutput.from_any(output.to_dict())

    assert rehydrated is not None
    assert isinstance(rehydrated.current_market_pressure, MarketPressureSnapshot)
    assert rehydrated.current_market_pressure.market_pressure_score == 42.0
    assert len(rehydrated.scenario_comparison) == 1
    assert isinstance(rehydrated.scenario_comparison[0], ScenarioComparisonResult)
    assert rehydrated.scenario_comparison[0].pressure is not None
    assert rehydrated.scenario_comparison[0].pressure.market_pressure_level == "L1_中性偏紧"
