...
def _probability_engine_scenario_artifacts(inp: DecisionCardBuildInput) -> dict[str, Any]:
    probability_engine_result = _obj(inp.probability_engine_result or {})
    probability_output = _obj(probability_engine_result.get("output") or {})
    ...
    explicit_ladder = [dict(_obj(item) or {}) for item in list(probability_output.get("scenario_ladder") or [])]
    if explicit_ladder:
...
def _attach_probability_engine_scenario_fields(rendered: dict[str, Any], inp: DecisionCardBuildInput) -> dict[str, Any]:
    scenario_artifacts = _probability_engine_scenario_artifacts(inp)
    rendered["current_market_pressure"] = scenario_artifacts.get("current_market_pressure")
    rendered["scenario_comparison"] = list(scenario_artifacts.get("scenario_comparison") or [])
    rendered["scenario_ladder"] = list(scenario_artifacts.get("scenario_ladder") or [])
    probability_explanation = dict(rendered.get("probability_explanation") or {})
    if probability_explanation or inp.probability_engine_result is not None:
        probability_explanation["scenario_ladder"] = list(scenario_artifacts.get("scenario_ladder") or [])
